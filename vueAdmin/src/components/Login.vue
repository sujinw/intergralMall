...
<script>
import api from '../vuex/modules/login/api.js'
      ...
      handleSubmit2(ev) {
        ...
        this.$refs.ruleForm2.validate((valid) => {
          if (valid) {
            ...
            api.login(_this.ruleForm2.account,_this.ruleForm2.checkPass,function(data){
              console.log(data)
              if(data.code === 20001){
                util.sessionSet('adminUsername',_this.ruleForm2.account);
                _this.$store.commit('COM_CONF',{isLogin:true})
                _this.$message({
                  message: '恭喜你，登录成功！',
                  type: 'success'
                });
